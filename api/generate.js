...
// ─── Gerador de Imagens ──────────────────────────────────────────────────────
// Usa Pollinations.ai (FLUX) — 100% gratuito, sem chave de API.

async function handleGenerateImage(req, res, apiKey, payload) {
  const { prompt } = payload;
  if (!prompt) return res.status(400).json({ error: 'Missing prompt.' });

  // Monta a URL do Pollinations com FLUX
  const seed = Math.floor(Math.random() * 999999);
  const imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=1024&height=1024&model=flux&enhance=true&nologo=true&seed=${seed}`;

  // Faz o fetch da imagem e retorna como base64 para o frontend
  const imgRes = await fetch(imageUrl);
  if (!imgRes.ok) {
    return res.status(502).json({ error: `Pollinations error: ${imgRes.status}` });
  }
